...
class Bot {
      ...
      bot.ws.on("message", data => {
        ...
        if(payload.op == 10){
          ...
          bot.once = bot.once.filter(listener => {
            ...
            if(!listener.criteria.every(c => {
              const fac = c.factor.split(".");
              //console.log(fac, payload.d);
              //console.log(tmp);
              let tmp = payload.d;
              for(let f of fac) tmp = tmp?.[f];
              return tmp == c.value;
            })) return true;
            listener.resolve(payload.d);
            //setTimeout(listener.reject, 60000 * 15);
            return false;
          });
        }
      });
  ...
  /*commandsUpdateWithCom(id, token, message){
    return this.rest.post(
      "/interactions/" + id + "/" + token + "/callback",
      { "type": 7, "data": message }
    );
  ...
  listenOnce(event, criteria, callback){
    return new Promise((resolve, reject) => {
      this.once.push({event, criteria, resolve, reject});
    });
  }
